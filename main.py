...
def add_schedule(data: dict, con: sqlite3.Connection = Depends(get_db)):
    ...
    try:
        pill, period, duration, user = data["pill_name"], data["time_period"], data["duration_days"], data["user_id"]

        period_rounded = []

        for stamp in period:  # округление до 15 минут
            ...
            if minute % 15 == 0:  # если кратно 15 минутам, то просто добавляем как есть
                ...
                if minute == 60:
                    period_rounded.append(str(hour + 1) + ":00")  # добавили час
